...
const Planets = () => {
  ...
  const handleMouseOverLike = (index) => {
    setHoveredFavorites((prevHovered) => [...prevHovered.slice(0, index), true, ...prevHovered.slice(index + 1)]);
  };

  const handleMouseLeaveLike = (index) => {
    setHoveredFavorites((prevHovered) => [...prevHovered.slice(0, index), false, ...prevHovered.slice(index + 1)]);
  };
      ...
      <div className="d-flex flex-row overflow-auto my-5 mx-5">
        {store.planets && store.planets.length > 0 && store.planets.map((item, index) => (
          <div className="cards mx-5" key={index}>
            <div
              onMouseOver={() => handleMouseOverLike(index)}
              onMouseLeave={() => handleMouseLeaveLike(index)}
              className="card mb-5"
              style={{ width: "20rem" }}
            >
              <img
                src={`https://starwars-visualguide.com/assets/img/planets/${item.url.split("/")[5]}.jpg`}
                className={hoveredFavorites[index] ? "card-img-top glow" : "card-img-top"}
                alt="..."
                onError={(e) => {
              ...
              <div className={hoveredFavorites[index] ? "card-body glow" : "card-body"}>
                ...
                <div className="buttons">
                  <Link to={"/singlePlanet/" + item.url.split("/")[5]} type="button" className="btn btn-outline-primary">
                    Learn more!
                  </Link>
                  <div className="buttonContainer">
                    ...
                    >
                      <FontAwesomeIcon
                        icon={faHeart}
                        className={hoveredFavorites[index] ? "heart-icon glow" : "heart-icon"}
                      />
                    </button>
                  </div>
                </div>
